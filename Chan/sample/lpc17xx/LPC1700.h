...
#define	USE_USER_SERVICE	1	/* Enable service functions/macros for user mode task */
...
#define __set_irqn_priority(n,v) IPR[n] = (v) << 3
#define __set_faultn_priority(n,v) SHPR[(n) + 16] = (v) << 3
...
#define __set_irqn_priority_user(n,v) __set_scs_reg(&IPR[n], (v) << 3)
#define __set_faultn_priority_user(n,v) __set_scs_reg(&SHPR[(n) + 16], (v) << 3)
#endif
...
#define __STREXB(d,p)	({uint32_t __r; uint8_t __d=d;  asm ("STREXB %0,%2,[%1]\n" : "=r" (__r) : "r" (p), "r" (__d)); __r;})
#define __STREXH(d,p)	({uint32_t __r; uint16_t __d=d; asm ("STREXH %0,%2,[%1]\n" : "=r" (__r) : "r" (p), "r" (__d)); __r;})
#define __STREXW(d,p)	({uint32_t __r, __d=d;          asm ("STREX  %0,%2,[%1]\n" : "=r" (__r) : "r" (p), "r" (__d)); __r;})
